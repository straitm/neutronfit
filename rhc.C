...
bool goodminos(const int nfreepar, const bool no_low_ok)
{
  for(int i = 0; i < nfreepar; i++)
    if((!no_low_ok && gMinuit->fErn[i] == 0) || gMinuit->fErp[i] == 0){
      printf("Incomplete MINOS errors for free par %d/%d\n", i, nfreepar);
      return false;
    }
  return true;
}
...
void fit(TF1 * ee, TH1D * hist, bool & n_good, bool & b12_good,
         double & n_mag, double & n_mage_up, double & n_mage_dn,
...
void rhc()
{
  ...
  TFile * f5 = new TFile("/nova/ana/users/mstrait/ndcosmic/all.periods235.type3.root", "Read");
  ...
  TFile * f6 = new TFile("/nova/ana/users/mstrait/ndcosmic/prod_pid_S16-12-07_nd_period6_keepup/754.root-type3.root"                          , "Read");
  ...
  const int nbins_e = 5;
  const double bins_e[nbins_e+1] = {0.5, 1.25, 2.0, 3.0, 4.0, 5 };

  TH2D * dum = new TH2D("dum", "", 100, 0, bins_e[nbins_e], 1000, 0, 10);

  TH2D * fhc_s = new TH2D("fhc_s", "", 680, -210, 470, nbins_e, bins_e);
  TH2D * rhc_s = (TH2D *)fhc_s->Clone("rhc_s");

  const char * const basecut =
    "primary && type == 3 && timeleft > 270 && timeback > 210 "
    "&& remid > 0.9 "
    "&& trklen > 200 " // standard
    //"&& trklen > 300 " // longer
  ...
  */
  ...
  for(int s = 1; s <= nbins_e; s++){
    ...
    const double n_rat_err_up =
      ratio_error(rhc_nmag, fhc_nmag, rhc_nmage_up, fhc_nmage_dn);
    const double n_rat_err_dn =
      ratio_error(rhc_nmag, fhc_nmag, rhc_nmage_dn, fhc_nmage_up);

    const double b12_rat     = rhc_b12mag/fhc_b12mag;
    const double b12_rat_err_up =
      ratio_error(rhc_b12mag, fhc_b12mag, rhc_b12mage_up, fhc_b12mage_dn);
    const double b12_rat_err_dn =
      ratio_error(rhc_b12mag, fhc_b12mag, rhc_b12mage_dn, fhc_b12mage_up);
    ...
    printf("%s/%s RHC/FHC neutron (%4.2f-%4.2f)GeV: %.3f + %.3f - %.3f\n",
      n_goodrhc?"Good":"Bad", n_goodfhc?"Good":"Bad", loslce, hislce,
    ...
    printf("%s/%s RHC/FHC B-12    (%4.2f-%4.2f)GeV: %.3f + %.3f - %.3f\n",
      b12_goodrhc?"Good":"Bad", b12_goodfhc?"Good":"Bad", loslce, hislce,
